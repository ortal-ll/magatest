...
describe('diagrams', () => {
  ...
  it('builds low-pass T filter', () => {
    const html = buildLowpassTFilter();
    assert.match(html, /100 мГн/);
  ...
  it('builds sync armature reaction Fa diagrams', () => {
    const html = buildSyncArmatureReactionFa();
    assert.match(html, />1</);
    assert.match(html, />2</);
    assert.match(html, />3</);
    assert.match(html, /Fₐ/);
    assert.match(html, /Φ_fmax/);
    assert.match(html, />d</);
    assert.match(html, />q</);
  });
