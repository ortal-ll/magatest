...
4. Сессия пишется в Upstash Redis, в `localStorage` и в cookie браузера на **3 месяца** (повторный ввод не нужен)
5. Без сессии любой прямой URL (`index`, `subject`, `quiz`, `/data/*`) возвращает на ввод кода:
   - **Netlify Edge** проверяет cookie на сервере
   - клиентский gate + скрытие страницы до проверки Redis

Без валидной сессии API результатов и статистики отвечает `403`.
